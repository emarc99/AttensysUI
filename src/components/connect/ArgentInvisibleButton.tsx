...
export const ArgentInvisibleButton: React.FC<ArgentInvisibleButtonProps> = ({ className }) => {
  const { account, connectStatus, invisibleConnect, disconnect } = useArgentInvisible();
  const [showTooltip, setShowTooltip] = useState(false);

  const handleConnect = async () => {
    await invisibleConnect();
  };
  ...
  return (
    <div>
      {!isConnected ? (
        <button
          className={`bg-gradient-to-r from-[#4A90E2] to-[#9B51E0] text-white px-4 py-2 font-semibold text-sm rounded-md flex items-center justify-center space-x-2 ${className}`}
        ...
        >
          ...
          <span>{isConnecting ? "Connecting..." : "Connect with Argent"}</span>
        </button>
      ) : (
