...
import { Button, Dialog, DialogBackdrop, DialogPanel, Field, Input } from '@headlessui/react';
...
export default function UploadModal(prop: any) {
  ...
  const [uploadStatus, setUploadStatus] = useState({
    video: { success: false, error: null, showMessage: false, progress: 0 } as UploadStatus,
    thumbnail: { success: false, error: null, showMessage: false, progress: 0 } as UploadStatus,
  });
  ...
  const handleFileUpload = async (file: File, type: 'video' | 'thumbnail') => {
    ...
    try {
      ...
      setUploadStatus((prev) => ({
        ...prev,
        [type]: { success: true, error: null, showMessage: true, progress: 100 },
      }));
      ...
      setUploadStatus((prev) => ({
        ...prev,
        [type]: { success: false, error: error.message, showMessage: true, progress: 0 },
      }));
  ...
  const handleFileSelect = (e: ChangeEvent<HTMLInputElement>, type: 'video' | 'thumbnail') => {
  ...
  return (
    <Dialog open={open} onClose={setOpen} className="relative z-10">
      <DialogBackdrop transition className="fixed inset-0 bg-[#0F0E0E82] transition-opacity" />
        ...
        <div className="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
          <DialogPanel className="relative h-[800px] w-[988px] transform overflow-hidden rounded-lg bg-white text-left shadow-xl sm:my-8">
            <div className="px-10 flex justify-between pt-10 cursor-pointer">
              <h1 className="text-[22px] font-semibold leading-[31px] text-[#5801A9]">Course Outline Upload</h1>
              <Image src={cancel} alt="cancel" onClick={() => { setOpen(false); setAddclass(false); }} />
            </div>
            ...
            <div className="w-[90%] mx-auto h-[75%] my-auto border-[1px] bg-[#EFEFEF52] border-[#C2C2C2] rounded-xl mt-7 px-8 py-6 space-y-4 overflow-y-scroll">
              ...
              <div className="space-y-2">
                <h1 className='text-[16px] text-[#2D3A4B] font-light leading-[23px]'>Topic: </h1>
                <Field>
                  <Input
                    name="topic"
                    ...
                    className={clsx(
                      'h-[55px] border-[2px] border-[#D0D5DD] block w-[50%] rounded-lg bg-white/5 py-1.5 px-3 text-sm/6 text-[#667185]',
              ...
              <div className="space-y-3">
                <h1 className='text-[16px] text-[#2D3A4B] font-light leading-[23px]'>Class Description</h1>
                <Field>
                  <textarea
                    name="description"
                    ...
                    className={clsx(
                      'h-[126px] border-[2px] bg-[#FFFFFF] border-[#D0D5DD] block w-[90%] rounded-lg py-1.5 px-3 text-sm/6 text-[#667185]',
                      'focus:outline-none data-[focus]:outline-2 data-[focus]:-outline-offset-2 data-[focus]:outline-white/25'
                    )}
                  />
                </Field>
              ...
              <div className="space-y-3">
                <h1 className='text-[16px] text-[#2D3A4B] font-light leading-[23px]'>Assignment</h1>
                <Field>
                  <textarea
                    name="assignment"
                ...
                <div className='flex space-x-10'>
                            <div className='cursor-pointer relative flex items-center space-x-3 w-[130px]'>
                                            ...
                                            <h1 className='text-[15px] leading-[18px] font-light text-[#5801A9]'>Resources</h1>
                            </div>
                            <div className='cursor-pointer relative flex items-center space-x-3 w-[130px]'>
                                    <FaPlus className='text-[#5801A9]' />
                                    <h1 className='text-[15px] leading-[18px] font-light text-[#5801A9]'>Assignment</h1>
                            </div>
                    </div>
              </div>
              ...
              <div className="mt-6">
                <h1 className="text-[16px] leading-[23px] font-semibold text-[#2D3A4B]">Upload Recording</h1>
                <div className="flex space-x-16">
                  <div
                    className="h-[316px] w-[459px] mt-3 rounded-lg bg-[#DCDCDC] flex justify-center items-center"
                    onDrop={(e) => handleDrop(e, 'video')}
                    onDragOver={(e) => e.preventDefault()}
                    onClick={() => videoInputRef.current?.click()}
                  >
                    <div className="h-[246px] w-[400px] border-[3px] border-dotted border-[#D0D5DD] bg-[#FFFFFF] rounded-xl space-y-3 flex flex-col items-center justify-center relative">
                      <Image src={cloud} alt="upload" />
                      <h1 className="text-[14px] text-[#475367] font-light leading-[20px]">
                        <span className="text-[#4A90E2]">Click to upload</span> or drag and drop
                      </h1>
                      ...
                      />
                      <Button
                        disabled={uploadStatus.video.progress > 0 && uploadStatus.video.progress < 100}
                  ...
                  <div className="mt-3 space-y-2">
                    <h1 className="w-[254px] text-[14px] font-light text-[#2D3A4B] leading-[21px]">
                      Upload your Recorded class here for your students to watch later
                    </h1>
                    <div
                    ...
                    >
                      ...
                      />
                      <Button
                        disabled={uploadStatus.thumbnail.progress > 0 && uploadStatus.thumbnail.progress < 100}
                      ...
                      >
                        {uploadStatus.thumbnail.progress > 0 && uploadStatus.thumbnail.progress < 100
                      ...
                      {uploadStatus.thumbnail.progress > 0 && uploadStatus.thumbnail.progress < 100 && (
                        <div className="w-[90%]  bg-gray-200 rounded-full h-2.5 ">
                          <div
                            className="bg-green-600 h-2.5  rounded-full"
                            style={{ width: `${uploadStatus.thumbnail.progress}%` }}
                          ></div>
                        </div>
                      )}
                    </div>
            ...
            <div className="flex justify-end my-5 px-16">
              ...
              >
                <h1 className="text-[#FFFFFF] text-[14px] font-semibold leading-[16px]">Save updates</h1>
              </div>
            </div>
          </DialogPanel>
        </div>
