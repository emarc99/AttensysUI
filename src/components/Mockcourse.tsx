...
export default function Mockevent() {
    ...
    const eventContract = new Contract(attensysCourseAbi, attensysCourseAddress, provider);
  ...
  const handleCreateCourse = async() =>{
    ...
    const myCall = eventContract.populate('create_course', ["0x05a679d1e0d9f67370d8c3250388afec2da1deaf895b51841e017a3eb7bfd154",1,"QmXGUbN7ccNtieggpCMTEQfnTqSP6Fb858sucaN2hjRsyv", "kennynft","knt"]);
  ...
  const handleCourseAdd = async() =>{
    eventContract.connect(wallet?.account);
    const myCall = eventContract.populate('add_replace_course_content', [2,"0x05a679d1e0d9f67370d8c3250388afec2da1deaf895b51841e017a3eb7bfd154","QmXGUbN7ccNtieggpCMTE", "QfnTqSP6Fb858sucaN2hjRsyv"]);
  ...
  const handleCourseFinish = async () => {
    eventContract.connect(wallet?.account);
    const myCall = eventContract.populate('finish_course_claim_certification', [1]);
    const res = await eventContract.finish_course_claim_certification(myCall.calldata);
    await provider.waitForTransaction(res.transaction_hash);
    console.log("Course added, transaction hash:", res.transaction_hash);
  }
  ...
  const get_user_completed_course = async () => {
    let user_completed_course_infos = await eventContract.get_user_completed_courses("0x05a679d1e0d9f67370d8c3250388afec2da1deaf895b51841e017a3eb7bfd154");
  ...
  const get_all_creator_courses = async () => {
    let all_creator_course_infos = await eventContract.get_all_creator_courses("0x05a679d1e0d9f67370d8c3250388afec2da1deaf895b51841e017a3eb7bfd154");
    console.log("All creator Course infos here", all_creator_course_infos);
  }

  const get_creator_info = async () => {
    let creator_info = await eventContract.get_creator_info("0x05a679d1e0d9f67370d8c3250388afec2da1deaf895b51841e017a3eb7bfd154");
 ...
 const get_complete_stat = async () => {
    let complete_stat = await eventContract.check_course_completion_status_n_certification(1,"0x05a679d1e0d9f67370d8c3250388afec2da1deaf895b51841e017a3eb7bfd154");
    ...
    return (
      <div>
        <div>
        <h1 className="text-3xl font-bold underline text-red-700" >mock course interaction here</h1>
        </div>

        <div>
        <button onClick={handleCreateCourse} className="border-black bg-red-500 mt-2">
        ...
        <div>
        <button onClick={handleCourseAdd} className="border-black bg-red-500 mt-2">
        Course Add button 
        </button>
        </div>

        <div>
        <button onClick={handleCourseFinish} className="border-black bg-red-500 mt-2">
