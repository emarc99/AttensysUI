...
export default function Mockevent() {
  ...
  const handleRegisterEvent = async () => {
    ...
    await provider.waitForTransaction(res.transaction_hash);
    console.log("Registration successful, transaction hash:", res.transaction_hash)
  }
  ...
  const handleMarkEventAttendance = async () => {
    ...
    await provider.waitForTransaction(res.transaction_hash);
    console.log("Attendance marked successfully, transaction hash:", res.transaction_hash)
  }
  ...
  const handleEventBatchCertify = async () => {
    ...
    await provider.waitForTransaction(res.transaction_hash);
    console.log("success batch certification complete, transaction hash:", res.transaction_hash)
  }
  ...
  return (
    ...
    <div>
    <button onClick={handleCreateEvent} className="border-black bg-red-500 mt-2">
        Create event button
    </button>
    </div>

    <div>
    <button onClick={handleRegisterEvent} className="border-black bg-red-500 mt-2">
        Register for event button
    </button>
    </div>

    <div>
    <button onClick={handleMarkEventAttendance} className="border-black bg-red-500 mt-2">
        Mark attendance for event button
    </button>
    </div>

    <div>
    <button onClick={handleEventBatchCertify} className="border-black bg-red-500 mt-2">
        batch certify event proof button
    </button>
    </div>
    <div>
    <button onClick={handleEndEvent} className="border-black bg-red-500 mt-2">
        End event button
    </button>
    </div>

    <div>
    <button onClick={handleEventRegCommencement} className="border-black bg-red-500 mt-2">
