        ...
        <table className="w-full border-collapse">
          <thead className=''>
            <tr className="border-b border-gray-300 h-[40px] text-[14px] text-[#333333]">
              ...
              <th className="text-left  px-4 leading-[22px] font-bold">Wallet Address</th>
              <th className="text-left px-4 leading-[22px] font-bold"> Roles</th>
              <th className="text-left px-4 leading-[22px] font-bold">Actions</th>
            </tr>
          </thead>
          <tbody>
      {permisiondata.map((data, index)=>{
            return  <tr key={index} className="border-b border-gray-300 h-[70px]">
              ...
              <td className="py-3 px-4 flex space-x-1 items-center h-[70px]">
                  ...
                  <div className='text-[12px] leading-[22px] font-medium text-[#FFFFFF] h-[28px] w-[58px] bg-[rgb(74,144,226)] rounded-lg flex items-center justify-center'>Owner</div>
                  <div className='text-[12px] leading-[22px] font-medium text-[#FFFFFF] h-[28px] w-[77px] bg-[#A666E3] rounded-lg flex items-center justify-center'>Instructor</div>
                  </> : <div className='text-[12px] leading-[22px] font-medium text-[#FFFFFF] h-[28px] w-[77px] bg-[#A666E3] rounded-lg flex items-center justify-center'>{data.access[0]}</div> }
              </td>
                    ...
                    <div className='flex flex-col justify-end items-start gap-2'>
                      {
                        data.access.map((role, index) => (
                          <div key={index} className={`${role == 'Owner' ? 'bg-[rgb(74,144,226)]' : 'bg-[#A666E3]'} text-white px-2 py-1 rounded-lg text-[12px]`}>
                            {role}
                          </div>
                        ))
                      }
