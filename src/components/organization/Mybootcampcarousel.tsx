...
const Mybootcampcarousel = (props : any) => {
  ...
  return (
    ...
    <Image src={logoImagesource} alt='eventimage' className='h-full w-full object-cover rounded-2xl' layout='fill' />
        </div>
    ...
    <div className='absolute bottom-0 z-20 w-full flex items-center justify-center text-center bg-carousell-gradient pb-4'>
        ...
        <Image src={NFTImagesource} alt='logo' className='object-cover h-full w-full rounded-full' height={24} width={24}/>
        </div>
        <div >
        <h1 className='text-[#FFFFFF] text-[10px] font-bold leading-[22px]'>{props.name}</h1>
        <h1 className='text-[#FFFFFF] text-[10px] font-medium leading-[8px]'>{date}</h1>
        </div>
        </div>
    </div>
</div>
